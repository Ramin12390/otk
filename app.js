...
const MISTRAL_API_KEY = "GdoAKWT0yIylegABxwYEu3mWebeMvZd5";
const MISTRAL_MODEL = "mistral-large-latest"; // Mətni ən dərindən analiz edən ən güclü model
const MISTRAL_VISION_MODEL = "pixtral-large-latest"; // Şəkilli sualları (Vision) yoxlayan model
...
CAVABIN YALNIZ VƏ YALNIZ JSON OBJECT FORMATINDA OLSUN. Formasi belə olmalıdır: { "questions": [ {..sual1..}, {..sual2..} ] }
...
RESPOND ONLY AS A VALID JSON OBJECT. Format: { "questions": [ {..q1..}, {..q2..} ] }. NO MARKDOWN TEXT AROUND IT.
...
Open Text example: {"id": 78, "subject": "İngilis dili", "type": "open_text", "text": "...", "points": 2}`;
        ...
        // Asinxron fetch funksiyası (Mistral API üçün)
        async function fetchSubjectQuestions(promptContent) {
            ...
            if (!data.choices || data.choices.length === 0) throw new Error("AI hissəvi cavab qaytarmadı.");
...
async function finishExam() {
    ...
    // 2. AI Submission (Mistral API)
    // Açıq sualların və şəkillərin Mistral/Pixtral API ilə yoxlanması
    for (let index = 0; index < questions.length; index++) {
        const q = questions[index];
        if (q.type !== "closed" && answers[index] && answers[index].trim().length > 0) {
            try {
                let promptText = `Sual: ${q.text}\nŞagirdin cavabı: ${answers[index]}\nBu sualın maksimal balı ${q.points}-dır. Şagirdin verdiyi cavabı (və varsa şəklini) yoxla və 0 ilə ${q.points} arasında bal ver. Yalnız və yalnız balı rəqəm olaraq qaytar (məsələn: 2 və ya 3.5). Başqa heç bir kəlmə yazma.`;
                ...
                if (q.type === "open_solution" && uploadedImages[index]) {
                    isVision = true;
                    const base64Image = uploadedImages[index]; // e.g. "data:image/jpeg;base64,..."
                ...
                // Şəkillilər üçün pixtral-large-latest, ancaq mətndirsə mistral-large-latest istifadə et
                const gradingModel = isVision ? MISTRAL_VISION_MODEL : MISTRAL_MODEL;
                ...
                console.error("Mistral API error for question " + index, err);
